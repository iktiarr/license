      ...
      <div className="flex items-center justify-between border-b border-zinc-800 pb-4">
        <div>
          <div className="flex items-center gap-2 text-xs text-zinc-400">
            ...
            <span className="text-xs text-zinc-300 font-semibold">./overview.sh</span>
        ...
        {[
          ...
          <div key={label} className="border border-zinc-800 rounded bg-zinc-950 p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-semibold text-zinc-600 tracking-widest">{label}</span>
              <span className={`w-1.5 h-1.5 rounded-full ${dot}`} />
            </div>
            <p className={`text-4xl font-extrabold tracking-tight ${color} mb-1`}>{value}</p>
            <div className="flex items-center justify-between">
              <span className="text-xs text-zinc-700">{desc}</span>
        ...
        <div className="col-span-2 border border-zinc-800 rounded bg-zinc-950">
          ...
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-800 bg-zinc-900/60">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-zinc-400 tracking-wider">RECENT PROJECTS</span>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="min-w-0 pr-2">
                      ...
                      <p className="text-xs text-zinc-600 truncate mt-0.5">{project.domain}</p>
                    </div>
        ...
        <div className="col-span-3 border border-zinc-800 rounded bg-zinc-950">
          ...
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-800 bg-zinc-900/60">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-zinc-400 tracking-wider">ACTIVITY STREAM</span>
            </div>
              ...
              stats.recentLogs.map((log) => {
                ...
                return (
                  <div key={log.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-zinc-900/30 transition-colors">
                    ...
                    {/* Time */}
                    <span className="text-xs text-zinc-700 shrink-0 font-mono">{timeStr}</span>
                  </div>
                );
              })
            )}
