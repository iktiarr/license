    ...
    <aside className="w-[240px] shrink-0 h-screen bg-black border-r border-zinc-800 flex flex-col justify-between select-none font-mono">
      ...
      <div>
        ...
        <div className="px-4 py-3 border-b border-zinc-800/60">
          <p className="text-[10px] text-zinc-500 uppercase tracking-widest mb-1">System</p>
          <p className="text-xs text-emerald-400 font-semibold tracking-wide">License Guard v1.0</p>
          <p className="text-[10px] text-zinc-600 mt-0.5">Control Hub &bull; Admin Access</p>
        </div>

        {/* Navigation Menu */}
        <nav className="p-3 space-y-0.5">
          <p className="px-2.5 py-2 text-[10px] font-semibold text-zinc-600 uppercase tracking-widest">
            // navigation
          </p>
          {navItems.map(({ href, label, icon: Icon, cmd }) => {
            ...
            return (
              <Link
                ...
                href={href}
                className={cn(
                  'flex items-center gap-2.5 px-3 py-2.5 rounded text-xs transition-all duration-150 group',
                  isActive
              ...
              >
                ...
                <Icon className={cn('w-3.5 h-3.5 shrink-0', isActive ? 'text-emerald-400' : 'text-zinc-600 group-hover:text-zinc-400')} />
      ...
      <div className="border-t border-zinc-800 p-3 space-y-2">
        ...
        <div className="flex items-center gap-2.5 px-2.5 py-2 rounded bg-zinc-900/50 border border-zinc-800/60">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[11px] font-semibold text-zinc-300 truncate">admin@guard</p>
            <p className="text-[9px] text-zinc-600 truncate">root privileges</p>
          </div>
        ...
        <form action="/api/auth/signout" method="POST">
          <button
            type="submit"
            className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded text-[11px] text-zinc-600 hover:text-rose-400 hover:bg-rose-500/10 border border-transparent hover:border-rose-500/20 transition-all cursor-pointer tracking-wider"
          >
